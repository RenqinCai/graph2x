...
def ray_best_rouge(index, test_reviews, dataset_name, subset_name):
    ...
    train_id2sentence_filepath = '../Dataset/ratebeer/{}/train/sentence/id2sentence.json'.format(dataset_name)
    ...
    test_id2sentence_filepath = '../Dataset/ratebeer/{}/test/sentence/id2sentence.json'.format(dataset_name)
    ...
    for test_review_chunk in test_reviews:
        ...
        with open("./result_{0}_{1}/{2}.json".format(dataset_name, subset_name, index), 'a') as f_write:
...
if __name__ == "__main__":
    ...
    test_review = []
    file_path_0 = '../Dataset/ratebeer/{0}/{1}/useritem2sentids_test_multilines.json'.format(args.dataset, args.subset)
    cnt = 0
    ...
    bias_size = 0
    data_instance_num = min(len(test_review), window_size*num_threads)
    ...
    for i in range(num_threads):
        results_rouge.append(ray_best_rouge.remote(
            i, test_review[(bias_size+i*window_size):(bias_size+(i+1)*window_size)], args.dataset, args.subset)
        )
    ...
    with open('./result_{0}_{1}/rouge_score.txt'.format(args.dataset, args.subset), 'w') as fw:
