...
class EVAL(object):
    def __init__(self, vocab_obj, args, device, vocab2id_dict, id2vocab_dict, sid2wordids_dict):
        super().__init__()
        ...
        self.m_sid2wordids = sid2wordids_dict       # sid to word ids (list)
        self.m_vocab2id = vocab2id_dict             # word to id
        self.m_id2vocab = id2vocab_dict             # id to word
    ...
    def f_eval_new(self, train_data, eval_data):
        ...
        with torch.no_grad():
            ...
            for graph_batch in eval_data:
                ...
                # get batch's cdd sentences' ids
                cdd_sids_batch = graph_batch.s_rawid
                cdd_sids_batch_size = cdd_sids_batch.size(0)

                # convert cdd sids to word ids
                cdd_sent_wids_batch = []
                cdd_sids_num_batch = []
                max_words_num_batch = 0
                pad_token_id = self.m_vocab2id['<unk>']
                for sid in cdd_sids_batch:
                    sid = sid.item()
                    sid_wordids = self.m_sid2wordids[sid]
                    max_words_num_batch = max(max_words_num_batch, len(sid_wordids))
                    cdd_sids_num_batch.append(len(sid_wordids))
                assert len(cdd_sids_num_batch) == cdd_sids_batch_size
                for sid in cdd_sids_batch:
                    sid = sid.item()
                    sid_wordids = self.m_sid2wordids[sid]
                    pad_word_num = max_words_num_batch - len(sid_wordids)
                    pad_wordids = [pad_token_id] * pad_word_num
                    sid_word_pad_ids = sid_wordids + pad_wordids
                    assert len(sid_word_pad_ids) == max_words_num_batch
                    cdd_sent_wids_batch.append(sid_word_pad_ids)

                # construct a mask matrix
                mask_sid_batch = torch.zeros(cdd_sids_batch_size, max_words_num_batch)
                for sid_i in range(cdd_sids_batch_size):
                    mask_sid_batch[sid_i][:cdd_sids_num_batch[sid_i]] = 1
                # shape: (sid_num, max_word_s, 1)
                mask_sid_batch = mask_sid_batch.unsqueeze(dim=-1)

                # construct a sid_length matrix
                sid_num_batch = torch.tensor(cdd_sids_num_batch)
                # shape: (sid_num, 1)
                sid_num_batch = sid_num_batch.unsqueeze(dim=-1)

                # put data to gpu
                cdd_sent_wids_batch_tensor = torch.LongTensor(cdd_sent_wids_batch)
                cdd_sent_wids_batch_tensor = cdd_sent_wids_batch_tensor.to(self.m_device)
                mask_sid_batch = mask_sid_batch.to(self.m_device)
                sid_num_batch = sid_num_batch.to(self.m_device)
                graph_batch = graph_batch.to(self.m_device)

                # logits: batch_size*max_sen_num
                s_logits, sids, s_masks, target_sids, f_logits, fids, f_masks, target_f_labels, hidden_f_batch = self.m_network.eval_forward(
                    graph_batch, cdd_sent_wids_batch_tensor, mask_sid_batch, sid_num_batch)
        ...
        if compute_rouge_score and compute_bleu_score:
            ...
            metric_log_file = os.path.join(self.m_eval_output_path, 'eval_metrics_{0}_{1}.txt'.format(self.m_dataset_name, label_format))
    ...
    def save_predict_sentences(self, true_userid, true_itemid, refs_sent, hyps_sent, topk_logits, pred_sids, top_cdd_logits, top_cdd_pred_sids, bottom_cdd_logits, bottom_cdd_pred_sids, s_topk_candidate=20):
        ...
        predict_log_file = os.path.join(self.m_eval_output_path, 'eval_logging_{0}_{1}.txt'.format(self.m_dataset_name, label_format))
        ...
        top_predict_log_file = os.path.join(self.m_eval_output_path, 'eval_logging_top_{0}_{1}.txt'.format(self.m_dataset_name, label_format))
        ...
        bottom_predict_log_file = os.path.join(self.m_eval_output_path, 'eval_logging_bottom_{0}_{1}.txt'.format(self.m_dataset_name, label_format))
