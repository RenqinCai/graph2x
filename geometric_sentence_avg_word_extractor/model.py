...
class GraphX(nn.Module):
    def __init__(self, args, vocab_obj, device, vocab2id_dict, vocab2embed_dict):
        super().__init__()
        ...
        self.m_word_finetune_flag = args.word_finetune
        ...
        self.m_vocab_size = len(vocab2embed_dict)
        self.m_word_embed = nn.Embedding(self.m_vocab_size, args.word_embed_size, padding_idx=vocab2id_dict['<unk>'])
        self.f_load_word_embedding(vocab2embed_dict)
        ...
        # sentence embedding is now the avg word embedding
        self.sent_state_proj = nn.Linear(args.word_embed_size, args.hidden_size, bias=False)
        ...
        self.m_gat = GATNET(
            ...
            dropout_rate=args.attn_dropout_rate
        )
    ...
    def f_load_word_embedding(self, pre_word_embed):

        pre_word_embed_weight = []
        print("pre_word_embed", len(pre_word_embed))

        for w_idx in range(self.m_vocab_size):
            word_embed_i = pre_word_embed[w_idx]
            pre_word_embed_weight.append(word_embed_i)

        self.m_word_embed.weight.data.copy_(torch.Tensor(pre_word_embed_weight))

        if not self.m_word_finetune_flag:
            self.m_word_embed.weight.requires_grad = False
    ...
    def forward(self, graph_batch, cdd_sent_wids_batch, mask_sid_batch, sid_num_batch):
        ...
        # get word embedding, shape: (sid_num, max_word_num_s, word_embed_size)
        s_word_embed = self.m_word_embed(cdd_sent_wids_batch)
        s_word_embed = s_word_embed * mask_sid_batch
        s_word_embed = torch.sum(s_word_embed, dim=1)  # shape: (sid_num, word_embed_size)
        s_word_embed = s_word_embed / sid_num_batch    # shape: (sid_num, word_embed_size)
        # get sentence node embedding
        s_node_embed = self.sent_state_proj(s_word_embed)
    ...
    def eval_forward(self, graph_batch, cdd_sent_wids_batch, mask_sid_batch, sid_num_batch, get_embedding=False):
        ## init node embeddings
