...
class TRAINER(object):
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj):
        ...
        for g_batch in train_data:
            ...
            # # labels_s_batch, _ = to_dense_batch(labels_s, graph_batch.batch)
            # for j in range(batch_size):
            #     print("User: {} \t Item: {}".format(userid[j].item(), itemid[j].item()))
            #     g = graph_batch[j]
            #     s_j = g.s_label
            #     print(s_j)
            # exit()
