...
class TRAINER(object):
    ...
    def __init__(self, vocab_obj, args, device, vocab2id_dict, id2vocab_dict, sid2wordids_dict):
        ...
        self.m_vocab2id = vocab2id_dict
        self.m_id2vocab = id2vocab_dict
        self.m_sid2swords = vocab_obj.m_sid2swords
        self.m_sid2wordids = sid2wordids_dict
        ...
        # the weight for the feature loss
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj):
        ...
        for g_batch in train_data:
            ...
            cdd_sids_batch = g_batch.s_rawid
            ...
            for i in range(cdd_sids_batch_size):
                mask_sid_batch[i][:cdd_sids_num_batch[i]] = 1
            ...
            if iteration % self.m_print_interval == 0:
                logger_obj.f_add_output2IO(
                    "%d, loss:%.4f, sent loss:%.4f, weighted feat loss:%.4f, feat loss:%.4f" % (
                        iteration,
                        np.mean(tmp_loss_list),
                        np.mean(tmp_loss_s_list),
                        feat_loss_weight*np.mean(tmp_loss_f_list),
                        np.mean(tmp_loss_f_list)
                        )
                    )
        ...
        logger_obj.f_add_output2IO(
            "%d, loss:%.4f, sent loss:%.4f, weighted feat loss:%.4f, feat loss:%.4f" % (
                ...
                np.mean(loss_list),
                np.mean(loss_s_list),
                feat_loss_weight*np.mean(loss_f_list),
                np.mean(loss_f_list)
                )
            )
    ...
    def f_eval_epoch(self, eval_data, network, optimizer, logger_obj):
        ...
        with torch.no_grad():
            for graph_batch in eval_data:
                ...
                s_logits, sids, masks, target_sids, _, _, _, _, _ = network.eval_forward(
                    graph_batch, cdd_sent_wids_batch_tensor, mask_sid_batch, sid_num_batch)
